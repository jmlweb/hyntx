...
describe('model-suggester', () => {
  describe('suggestBestModel', () => {
    it('should suggest qwen2.5:14b when available with 8GB+ RAM', () => {
      ...
      const models: OllamaModelInfo[] = [
        { name: 'gemma4:e4b:latest', size: 5000000000 },
    ...
    it('should suggest mistral:7b when qwen not available but mistral is, with 8GB+ RAM', () => {
      ...
      expect(result.reason).toContain('Full Schema');
    });

    it('should suggest gemma4:e4b when available with low RAM', () => {
      ...
      expect(result.suggestedModel).toBe('gemma4:e4b');
      expect(result.reason).toContain('128K context');
    });

    it('should fallback to gemma4:e4b when no models available', () => {
      const systemInfo: SystemInfo = { ramGB: 16 };
      const models: OllamaModelInfo[] = [];
    ...
    it('should fallback to gemma4:e4b when only unknown models available', () => {
      const systemInfo: SystemInfo = { ramGB: 16 };
      const models: OllamaModelInfo[] = [
    ...
    it('should suggest gemma4:e4b with exactly 8GB RAM when only gemma4:e4b available', () => {
  ...
  describe('getInstallationSuggestion', () => {
    ...
    it('should suggest gemma4:e4b with 4-7GB RAM when not installed', () => {
      ...
      expect(result).not.toBeNull();
      expect(result?.suggestedModel).toBe('gemma4:e4b');
      expect(result?.benefits).toContain('128K context');
    });
  ...
  describe('edge cases', () => {
    it('should handle empty models list gracefully', () => {
      ...
      expect(modelSuggestion.suggestedModel).toBe('gemma4:e4b');
